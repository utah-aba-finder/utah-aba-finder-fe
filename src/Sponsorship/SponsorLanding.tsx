...
import { CheckCircle2, Sparkles, ShieldCheck, Star, HeartHandshake, ArrowRight, BadgeInfo, Mail } from "lucide-react";
...
const TierCard: React.FC<{
  ...
  onSelect: () => void;
  isLoading?: boolean;
}> = ({ title, price, tagline, features, highlight = false, badge, onSelect, isLoading = false }) => (
  <motion.div
  ...
  >
    ...
    <div className="mb-4 flex items-center gap-3">
      {highlight ? <ShieldCheck className="h-6 w-6 text-teal-600" /> : <Star className="h-6 w-6 text-zinc-600" />}
...
export default function SponsorLanding() {
  ...
  async function createCheckout(plan: "featured" | "sponsor" | "partner") {
    try {
      setLoadingTier(plan);
        ...
        <Container className="py-16 sm:py-24">
          <div className="grid items-center gap-10 md:grid-cols-2">
            <div>
              ...
              >
                Become a <span className="text-teal-600">Sponsor Provider</span>
              </motion.h1>
              <p className="mt-4 max-w-prose text-zinc-700">
                Support families and grow your practice. Sponsor Providers are highlighted at the top of search results on
            ...
            >
              <div className="text-sm text-zinc-600">What you get</div>
              <ul className="mt-3 grid gap-3 text-sm">
                {[
                  "Top-of-list placement in your service area",
                  "Sponsor badge & logo on your listing",
                  "Featured Providers section visibility",
                  "Optional homepage recognition for partners",
                ].map((i, idx) => (
              ...
              <div className="mt-6 rounded-xl bg-teal-50 p-4 text-sm text-teal-900 ring-1 ring-inset ring-teal-200">
                Your sponsorship keeps our directory <strong>free for families</strong> nationwide.
              </div>
            </motion.div>
          </div>
        </Container>
        ...
        <Container className="py-12 sm:py-16">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            <TierCard
              ...
              onSelect={() => handleTierSelect("Featured Provider")}
            ...
            <TierCard
              ...
              highlight
              badge="Most popular"
              onSelect={() => handleTierSelect("Sponsor Provider")}
            ...
            <TierCard
              ...
              onSelect={() => handleTierSelect("Community Partner")}
              isLoading={loadingTier === "partner"}
            />
          </div>

          <div className="mt-8 rounded-2xl border border-zinc-200 bg-white p-5 text-sm text-zinc-700">
            <BadgeInfo className="mr-2 inline h-4 w-4 text-zinc-500" />
            Annual billing available (2 months free). Email us to switch after checkout.
          </div>
        </Container>
      </section>
